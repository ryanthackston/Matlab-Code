function [labels, meg, class_label, temp_ind] = meg_labels_aug2021(answers, blocks, stud, subj)

% take out initial 3000 points due to No Action

meg = [];
for b = 1:size(blocks,1)
    meg = [meg; blocks(b).meg(3101:end, :)];
end
        ...
        for L = 1:size(blocks,1)
            labels = [ labels; blocks(L).stateSignals.targetCode(3101:end, :) ];
        end
