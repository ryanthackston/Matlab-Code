function varargout = slidingPLV(varargin)
% SLIDINGPLV MATLAB code for slidingPLV.fig
%      SLIDINGPLV, by itself, creates a new SLIDINGPLV or raises the existing
%      singleton*.
%
%      H = SLIDINGPLV returns the handle to a new SLIDINGPLV or the handle to
%      the existing singleton*.
%
%      SLIDINGPLV('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SLIDINGPLV.M with the given input arguments.
%
%      SLIDINGPLV('Property','Value',...) creates a new SLIDINGPLV or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before slidingPLV_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to slidingPLV_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help slidingPLV

% Last Modified by GUIDE v2.5 21-Apr-2020 22:44:58

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @slidingPLV_OpeningFcn, ...
                   'gui_OutputFcn',  @slidingPLV_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before slidingPLV is made visible.
function slidingPLV_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to slidingPLV (see VARARGIN)
...
% Choose default command line output for slidingPLV
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes slidingPLV wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = slidingPLV_OutputFcn(hObject, eventdata, handles) 
