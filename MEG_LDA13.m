...
% window
% PLV_Rest_I(PLV_Rest_I == 0) = [];
% PLV_Move_I(PLV_Move_I == 0) = [];
% A = zeros(4996,1);
% for iii = 1:4996
%     A(iii) = sort(mode(I{iii}(1,:)));
% %     A(hist(A,A) >2)
% end
...
% Make cells of every channel combination
% chanx = string(1:chans);
% chany = string(1:chans);
% chan_inter = cell(chans, chans);
% for x = 1:chans
%     for y = 1:chans
%         chan_inter{y,x} = strcat("Channels ", chanx(x), "-", chany(y));
%     end
% end

% Channel interactions need to be low to high
% chan_inter = chan_inter(tril_ind);

Mahal_Vec = zeros(trials_compared*10, 3);
Class_Vec = cell( trials_compared * top_features, 1 );
for i = 1:trials_compared
    %Sorted Mahal Distances
    Mahal_Vec( (i*10-10+1): i*10, 1 ) = PLV_Mahal_Sort{i};
    %Column Coord Channels
    Mahal_Vec( (i*10-10+1): i*10, 2 ) = PLV_Mahal_Coord{i}(1:10,1);
    %Row Coord Channels
    Mahal_Vec( (i*10-10+1): i*10, 3 ) = PLV_Mahal_Coord{i}(1:10,1);
...
%I need to make cells of the channels
