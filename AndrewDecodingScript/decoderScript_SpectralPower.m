...
for blockIDX = 1:numBlocks
    ...
    %Extract the spectral power based on the loaded meg data
    alphaPowerMatrix = getWindowedSpectralPower(meg,...
        samplingRate,updateRate,windowLength,nfft,alphaFreqBand);
    betaPowerMatrix = getWindowedSpectralPower(meg,...
        samplingRate,updateRate,windowLength,nfft,betaFreqBand);
    ...
    featureMatrixBlocks{blockIDX} = ...
        [alphaPowerMatrix betaPowerMatrix];
    
    %Extract the class labels so taht they match the same rate as
