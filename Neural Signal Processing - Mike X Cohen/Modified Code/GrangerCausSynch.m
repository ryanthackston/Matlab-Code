...
load('C:\Users\ryanr\Downloads\Neural Signal Processing - Mike X Cohen\v1_laminar.mat')

% RestOnset & MoveOnset -> Time points of trial onsets
% stimulusCode -> 0 is non stimulus, 1 is rest, 2 is movement
% meg is 253000 data points x 18 channels
load('D:\OPM data - August 2019 - Colorado\BCI2000 mat files\imag_calibration\S9_Block2_imag_calib.mat')
meg(:,[4 6 7 11 15 16 20 23]) = [];
meg = meg';
megMat(1,:) = meg(12,:);
megMat(2,:) = meg(7,:);
megMat(3,:) = meg(6,:);
...
% figure(5); plot(time,real(wavelet))
...
nWave = length(time);
% Time vector for 1 trial of data
nData = size(csd,2);
% Length of convolution result
nConv = nWave + nData - 1;

% FFT of wavelet (check nfft)
% waveletX is the spectra on the wavelet used in convolution to multiply the data spectrum
waveletX = fft(wavelet,nConv);
waveletX = waveletX ./ max(waveletX);
% figure(5); plot(abs(waveletX))
% gives frequency index, we want a gaussian kernel
...
% analytic signal of channel 1 from trial 1
% fft of csd data with Gaussian Kernel -> (channel 1, all data, trial 1), nConv)
...
%figure(5); plot(timevec, phase_data)
...
phase_angle_differences = phase_data(1,:) - phase_data(2,:);
...
% check if diff(phase_data) is correct size -> 1x1527
% We want the phase difference between 2nd channel and 1st channel for each
% time point. Then we take the average of the phase differences and the
% absolute value of the result b/c the difference b/w numbers is positive.
...
phase_synchronization_backwards = abs(mean(exp(1i*-diff(phase_data))));
...
x = randn(N,1); % X is a totally random set of numbers
y = randn(2,1);

% new y
for t=2:N-1
    y(t+1) = a1*x(t) + a2*x(t-1); % Y is a totally deterministic system
end
