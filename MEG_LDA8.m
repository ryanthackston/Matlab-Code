...
% Delete first 3000 MEG Data Points in both blocks
    ...
    % meg low pass
       ...
       d = 'data';
       d_val = cell( ((size(meg, 2) - tw)/shift + 1)*2, 1 );
    ...
    %     x_lp_str_medavg = cell((size(meg, 2) - tw)/shift + 1, 2 );
       ...
       group = cell( 2 * ((size(meg, 2) - tw)/shift + 1), 1 );
...
%                ylabel('Power (something squared / Hz)');
               ...
               x_pow_str.data{i, 1} = [x_pow_str.alpha; x_pow_str.beta];

    % Group is the class labels for each alpha & beta value in all time
    % windows
%                 for j = 1:2
%                      group { ((i-1) * 2)+(j) } =  x_pow_str.data{i, 2} {j};
%                 end
              group{2*i-1, 1} = meg_tw{i,2};
              group{2*i, 1} = meg_tw{i,2};
...
% creates 5-fold data to test on
    [ testIdx, trainIdx, nTest] = cvpartition1(size(x_pow_str.alpha,1),5);
...
%     gs = kron(trainIdx,ones(1,6));
    ...
    for i = 1:5 
        ...
        [C{i, 1},err(i, 1),P{i, 1},logp{i, 1},coeff{i, 1}] = classify( [ cell2mat(x_pow_str.alpha(testIdx(i,:))); cell2mat(x_pow_str.beta(testIdx(i,:))) ], ...
                                                                    [cell2mat(x_pow_str.alpha(trainIdx(i,:))); cell2mat(x_pow_str.beta(trainIdx(i,:))) ] , ...                                                                  
                                                                    group(find([trainIdx(i,:) trainIdx(i,:)])), 'linear' );
